...
class DetectMultiBackend(nn.Module):
    ...
    def __init__(
        self, weights="yolov5s.pt", device=torch.device("cpu"), dnn=False, data=None, fp16=False
    ):
        # Usage:
        #   PyTorch:              weights = *.pt
        ...
        if pt:  # PyTorch
            ...
            if saved_model:  # SavedModel
                ...
                def wrap_frozen_graph(gd, inputs, outputs):
                    ...
                    return x.prune(
                        tf.nest.map_structure(ge, inputs), tf.nest.map_structure(ge, outputs)
                    )
...
class AutoShape(nn.Module):
    ...
    def forward(self, imgs, size=640, augment=False, profile=False):
        ...
        t = [time_sync()]
        p = (
            next(self.model.parameters()) if self.pt else torch.zeros(1, device=self.model.device)
        )  # for device, type
        ...
        # Pre-process
        n, imgs = (
            (len(imgs), list(imgs)) if isinstance(imgs, (list, tuple)) else (1, [imgs])
...
class Detections:
    ...
    def tolist(self):
        ...
        x = [
            Detections(
                [self.imgs[i]], [self.pred[i]], [self.files[i]], self.times, self.names, self.s
            )
            for i in r
        ]
        # for d in x:
